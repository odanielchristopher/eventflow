...
router = APIRouter(prefix="/events/{event_id}/check-ins", tags=["check-ins"])
...
async def get_check_in_by_id(
    ...
    return await usecase.execute(event_id, check_in_id)
...
async def update_check_in(
    ...
    return await usecase.execute(event_id, check_in_id, payload)
...
async def delete_check_in(
    ...
    await usecase.execute(event_id, check_in_id)
